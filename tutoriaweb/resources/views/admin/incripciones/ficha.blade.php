...
<div class="column is-12">
	...
	<input type="radio" id="responsable" name="nav-tab" onclick="ChResponsable()">
				...
				<form class="long-form">				
					<div class="columns">
						<div class="column is-one-fifth">
							...
							<input type="text" name="name" id="nombre" class="input {{ $errors->has('name') ? ' is-danger' : '' }}" value="{{ ($estudiante[0]->nombre) }}">
							<p class="help is-danger" id="ErrNombre" hidden>Campo obligatorio *</p>
						</div>

						<div class="column is-one-fifth">
							<label class="label">Primer apellido</label>
							<input type="text" name="apellido1" id="apellido1" class="input {{ $errors->has('apellido1') ? ' is-danger' : '' }}" value="{{ ($estudiante[0]->apellido1) }}">
							<p class="help is-danger" id="ErrApellido1" hidden>Campo obligatorio *</p>
						</div>

						<div class="column is-one-fifth">
							<label class="label">Segundo apellido</label>
							<input type="text" name="apellido2" id="apellido2" class="input {{ $errors->has('apellido2') ? ' is-danger' : '' }}" value="{{ ($estudiante[0]->apellido2) }}">
						</div>
							...
							<div class="select">
								<select name="tipgenero" id="tipgenero">
									...
									@foreach($generos->all() as $genero)
										@if ($genero->codigo === $estudiante[0]->genero)
											<option selected value="{{ $genero->codigo }}">{{ $genero->nombre }}</option>
										@else
							...
							<div class="select">
								<select name="tipdocu" id="tipdocu">
									...
									@foreach($tipodocumentos->all() as $tpdocu)
										@if ($tpdocu->codigo === $estudiante[0]->tipodocumento)
											<option selected value="{{ $tpdocu->codigo }}">{{ $tpdocu->nombre }}</option>
										@else
						...
						<div class="column is-one-fifth">
							...
							<input type="number" name="numdocu" id="numdocu" style="width: 113px;" class="input {{ $errors->has('numdocu') ? ' is-danger' : '' }}" value="{{ ($estudiante[0]->numdocumento) }}" disabled>
						</div>						
					</div>
					...
					<div class="columns">									
						<div class="column is-one-fifth">
							...
							<input type="text" name="direccion" id="direccion" class="input {{ $errors->has('direccion') ? ' is-danger' : '' }}" value="{{ ($estudiante[0]->direccion) }}">
							...
							<div class="select">
								<select name="barrio" id="barrio">
									...
									@foreach($barrios->all() as $barrio)
										@if ($barrio->cod_barrio === $estudiante[0]->barrio)
											<option selected value="{{ $barrio->cod_ciudad }}">{{ $barrio->nombre }}</option>
										@else										
						...
						<div class="column is-one-fifth" style="width: 157px">
							...
							<input type="number" name="numfijo" id="numfijo" class="input {{ $errors->has('numfijo') ? ' is-danger' : '' }}" value="{{ ($estudiante[0]->numtelefono) }}">
							<p class="help is-danger" id="ErrNumtel" hidden>Campo obligatorio *</p>
						</div>

						<div class="column is-one-fifth" style="width: 157px">
							<label class="label"># Celular</label>							
							<input type="number" name="numcelular" id="numcelular" class="input {{ $errors->has('numcelular') ? ' is-danger' : '' }}" value="{{ ($estudiante[0]->numcelular) }}">
							<p class="help is-danger" id="ErrNumcel" hidden>Campo obligatorio *</p>
						</div>	

						<div class="column is-one-quarter">
							<label class="label">Correo electrónico</label>
							<input type="email" id="email" class="input {{ $errors->has('email') ? ' is-danger' : '' }}" onchange="ValEmail();" value="{{ ($estudiante[0]->email) }}">
						...
						<div class="column is-one-fifth" style="width: 190px">
							...
							<input type="date" id="fecnaci" class="input {{ $errors->has('fecnaci') ? ' is-danger' : '' }}" max="<?php echo date('Y-m-d');?>" value="{{ ($estudiante[0]->fechanace) }}">
					...
					<div class="columns">
						<div class="column is-one-fifth" >
							...
							<input type="text" name="painace" id="painace" attr-value="CO" value="Colombiaaana" class="input {{ $errors->has('painace') ? ' is-danger' : '' }}">
			...
			<div class="tab-pane content-responsable">
				<form class="long-form">				
					<div class="columns">
						...
						<div class="column is-one-fifth">
							<label class="label"># Documento</label>
							<input type="number" name="numdocures" id="numdocures" class="input {{ $errors->has('numdocu') ? ' is-danger' : '' }}" disabled>
						</div>
					</div>
			...
			<div class="tab-pane content-acudiente">
				<form class="long-form">
					<div class="columns">
						...
						<div class="column is-one-fifth">
							<label class="label"># Documento</label>
							<input type="number" name="numdocuacu" id="numdocuacu" class="input {{ $errors->has('numdocu') ? ' is-danger' : '' }}" disabled>
						</div>
					</div>
...
<script>
	...
	var Paisnace = '<?php echo $estudiante[0]->cod_pais_nace;?>';
	for (var i=0; i<Arrpaises.length ; i++){		
		if(Paisnace === Arrpaises[i].codigo){
			$("#painace").attr('attr-value',Arrpaises[i].codigo);
			$("#painace").attr('value',Arrpaises[i].nombre);
		}
	}
	
	var Ciunace = '<?php echo $estudiante[0]->cod_ciu_nace;?>';
	for (var i=0; i<Arrcuidades.length ; i++){		
		if(Ciunace === Arrcuidades[i].cod_ciudad){
			$("#ciunace").attr('attr-value',Arrcuidades[i].cod_ciudad);
			$("#ciunace").attr('value',Arrcuidades[i].nombre);
		}
	}	
